...
cat ../bdd/lh-values.yaml.template >> env/lighthouse/values.tmpl.yaml

# Manually interpolate lighthouse-jx version tag
mkdir -p env/lighthouse-jx
cat ../bdd/values.yaml.template >> env/lighthouse-jx/values.tmpl.yaml
cp env/lighthouse-jx/values.tmpl.yaml values.tmpl.yaml.tmp
sed 's/$VERSION/'"$VERSION"'/' values.tmpl.yaml.tmp > env/lighthouse-jx/values.tmpl.yaml
cat env/lighthouse-jx/values.tmpl.yaml
rm values.tmpl.yaml.tmp
sed -e s/\$VERSION/${VERSION}/g ../bdd/helm-requirements.yaml.template > env/requirements.yaml

echo "Building lighthouse-jx with version $VERSION"
